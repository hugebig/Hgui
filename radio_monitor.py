...
class Radio_monitor(object):
	def __init__(self,the_dic):
		self.value = DoubleVar()
		self.title, self.dic,self.func = the_dic
		
	def show_info(self):
		the_value = self.value.get()
		for each in self.dic:
			if each.values()[0]==the_value:
				select =  each.keys()[0]
		text = "{0}\t{1}\t{2}".format(self.title,select,the_value)
		self.label['text'] = text
		self.func(text)

	def add_gui(self,form):
		self.form = form
		self.form.row()
		self.form.col()
		self.label = self.form.la(text=self.title,bg='SkyBlue',padx=2)
		self.form.gr(3)
		for info in self.dic:
			self.form.rb(text=info.keys()[0], variable=self.value, value=info.values()[0],\
		              command=self.show_info,indicatoron=1)
		self.form.endrow()
